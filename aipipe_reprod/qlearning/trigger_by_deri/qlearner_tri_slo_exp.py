...
class Qlearner:
    ...
    def train(self):
        ...
        for episode in range(self.n_episodes):
            ...
            if self.should_trigger(episode):
                dataset_desc = get_dataset_description(self.filename, self.dataset['train'])
                llm_pipelines = self.llm.get_multiple_pipelines_with_experience(dataset_desc, last_episode_pipelines, self.best_pipes[self.filename])

                for p in llm_pipelines:
                    metric, q_copy, step_rewards = self.run_full_pipe_and_calc_metrics_and_update_q(p.pipe)
            ...
            logger.info(f'{episode=}', f'by llm {by_llm}', best[0].downstream, best[2])
            ...
            if not found_flag and len(step_rewards) > 0:
                self.best_pipes[self.filename].append([
                    {"op_id": int(act), "op_name": str(QActionProvider.get(act).get_name()), 'acc': float(accuracy), 'reward': float(reward)}
                    for (act, reward, accuracy) in step_rewards
                ])
                if len(self.best_pipes[self.filename]) > 0 and len(self.best_pipes[self.filename][0]) > 0:
    ...
    def run_full_pipe_and_calc_metrics_and_update_q(self, pipe: list[int]):
        ...
        try:
            # 保证最后一个是下游任务
            if self.n_actions - 1 not in pipe:
                pipe.append(self.n_actions - 1)
            ...
            return metrics, q_copy, step_rewards
    ...
    def infer(self, q):
        ...
        if not os.path.exists('save/using_aipipe'):
            os.makedirs('save/using_aipipe')
        with open(f'save/using_aipipe/trigger_slope_exp_{self.downstream_task.get_name()}_results.txt', 'a') as fout:
            print((
