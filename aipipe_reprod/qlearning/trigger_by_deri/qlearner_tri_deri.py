...
class Qlearner:
    ...
    def train(self):
        ...
        for episode in range(self.n_episodes):
            ...
            if self.should_trigger(episode):
                ...
                llm_pipelines = self.llm.get_multiple_pipelines_from_last_episodes(dataset_desc, last_episode_pipelines)

                for p in llm_pipelines:
                    metric, q_copy = self.run_full_pipe_and_calc_metrics_and_update_q(p.pipe)
            ...
            _, best = self.infer_partial(self.q, episode)
            print(f'{episode=}', f'by llm {by_llm}', best[0].downstream, best[2])
    ...
    def infer(self, q):
        ...
        if not os.path.exists('save/using_aipipe'):
            os.makedirs('save/using_aipipe')
        with open(f'save/using_aipipe/trigger_slope_inspect_trend_q_{self.downstream_task.get_name()}_results.txt', 'a') as fout:
            print((
                'trigger with slope',
                self.filename,
                self.target_name,
