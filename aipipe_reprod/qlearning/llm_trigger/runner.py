...
from aipipe_reprod.qlearning.llm_trigger.ql_re_llm_dis_tri_exp import Qlearner
...
for i in [2, 12]:
    ...
    ql = Qlearner(dataset, 'LogisticRegressionPrim', target, filename=filename, n_episodes=100, rand_seed=42,
                  weights={'downstream':0.7, 'quality': 0.05, 'cost': 0.01}, epsilon_decay=0.99)
    if os.path.exists(f'save/vector_store/advisorpp/index.faiss'):
        ql.semantic_search.load_vector_store('save/vector_store/advisorpp')
    ...
    today = datetime.now().strftime("%Y%m%d")
    now = datetime.now().strftime("%H%M%S")
    ql.semantic_search.save_vector_store(f'save/vector_store/advisor/advisor_split/{filename}_{today}_{now}')
