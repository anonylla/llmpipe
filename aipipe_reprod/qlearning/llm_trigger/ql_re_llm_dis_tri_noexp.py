...
class Qlearner:
    def __init__(self,
        ...
        # self.semantic_search = EmbeddingsManager()
    ...
    def generate_full_pipe(self, df: pd.DataFrame, 
        ...
        if semantic_best_pipes is not None and len(semantic_best_pipes) > 0:
            # best_pipe_str = 'Successful Pipeline or action on this dataset are: (you can refer to these pipelines and think of some possible good pipelines)\n'
            # for j, pipe in enumerate(semantic_best_pipes[:best_pipes_cnt]):
            #     best_pipe_str += f'\nExample {j + 1}: ['
            #     for i, op in enumerate(pipe):
            #         best_pipe_str += f'{op["op_id"]} {QActionProvider.get(op["op_id"]).get_name()}, accuracy {op["acc"]:.4f}'
            #         if i != len(pipe) - 1:
            #             best_pipe_str += '\n'
            #     best_pipe_str += ']\n'
    ...
    def infer(self, q):
        ...
        if not os.path.exists('save/using_aipipe'):
            os.makedirs('save/using_aipipe')
        with open(f'save/using_aipipe/re_llm_dis_tri_noexp_{self.downstream_task.get_name()}_results.txt', 'a') as fout:
            print((
                'llm distri trig no exp',
                self.filename,
                self.target_name,
