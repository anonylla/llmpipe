...
class Qlearner:
    def __init__(self,
        ...
        max_train_samples = train_len # min(400, int(train_len))
        max_test_samples = test_len #  min(100, int(test_len))
        ...
        self.best_pipes: dict[str, list[list[dict]]] = best_pipes
    ...
    def calculate_metrics_in_exploring(self):
        ...
        return Metrics(accuracy_metric, quality_metric, cost)
    ...
    def train(self, verbose_all_episodes=False):
        ...
        for episode in range(self.n_episodes):
            ...
            ### 添加 llm 生成的 distribution 暂时先不加，看看效果怎么样
            # if episode > 0 and self.llm_distri_frequ > 0 and episode % self.llm_distri_frequ == 0:
            #     self.llm_prob_distribution = get_last_episode_distribution(
            #         self.llm, self.filename, self.init_dataset['train'],
            #         self.pipeline, self.last_prep_metric.downstream - self.no_prep_metric.downstream, self.last_prep_metric.downstream
            #     )
            ...
            prob = self._format_prob(self.llm_prob_distribution)
            current_state = int(random_state.choice(states, p=prob))
            ...
            while current_state < n_states - 1:
                ...
                step_rewards.append((action, metrics.downstream - self.last_prep_metric.downstream, metrics.downstream))
                next_state = action
            ...
            if should_trigger_llm:
                ...
                temp_q = np.array(self.q)
                current_df = self.reset_dataset()   # TODO 这个对象是一个 object 而不是 data frame
                pipeline: list[int] = self.generater_full_pipe(current_df['train'], step_rewards, self.best_pipes.get(self.filename, []), best_pipes_cnt=3)
                if pipeline is None:
                    logger.warning('LLM did not provided a valid pipeline.')
                    continue

                if QActionProvider.done_action not in pipeline:
            ...
            if self.filename not in self.best_pipes:
                self.best_pipes[self.filename] = []
            found_flag = False
            for i, pipe in enumerate(self.best_pipes[self.filename]):
                if tuple(map(lambda x: x[0], step_rewards)) == tuple(map(lambda x: x['op_id'], pipe)):
                    found_flag = True
                    break
            if not found_flag and len(step_rewards) > 0:
                self.best_pipes[self.filename].append([
                    {"op_id": act, "op_name": QActionProvider.get(act).get_name(), 
                    'acc': accuracy, 'reward': reward}
                    for (act, reward, accuracy) in step_rewards
                ])
            if len(self.best_pipes[self.filename]) > 0:
                self.best_pipes[self.filename].sort(key=lambda x: x[-1]['acc'], reverse=True)
                self.best_pipes[self.filename] = self.best_pipes[self.filename][:10]
    ...
    def generater_full_pipe(self, df: pd.DataFrame, step_rewards: list[tuple[int, float, float]], best_pipes: list[list[dict[str, int | float]]]=[], best_pipes_cnt=3):
        dataset_desc = get_dataset_description(self.filename, df)
        actions_text = get_actions_description(list(range(self.n_actions)))

        # 总结最优的 top 3 pipeline 的字符串
        best_pipe_str = ''
        if best_pipes is not None and len(best_pipes) > 0:
            best_pipe_str = 'Successful Pipeline on this dataset are: (you can refer to these pipelines and think of some possible good pipelines)\n'
            for j, pipe in enumerate(best_pipes[:best_pipes_cnt]):
                first_str = f'(The best with final accuracy {pipe[-1]["acc"]:.4f})' if j == 0 else ''
                best_pipe_str += f'\nExample {j + 1} {first_str}: [\n'
                for i, op in enumerate(pipe):
                    best_pipe_str += f'Step {i}: {op["op_id"]} {QActionProvider.get(op["op_id"]).get_name()}, accuracy {op["acc"]:.4f}\n'
                best_pipe_str = best_pipe_str[:-2] + ']\n'
