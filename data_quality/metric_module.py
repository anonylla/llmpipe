...
def preProcess(df: pd.DataFrame, uniqueID):
...
def computeModeDeviation(dataframe: pd.DataFrame):
...
def iatRegularityMetricOld(dataframe: pd.DataFrame):
...
def iatRegularityMetric(dataframe: pd.DataFrame):
...
def iatOutliersMetric(dataframe: pd.DataFrame):
    df = dataframe
    data = df['IAT'].dropna()
    # print(Q1, Q3)
...
def duplicatesMetric(df: pd.DataFrame, input1, input2):
...
def validate_data_with_schema(dataF: str, schema):
    num_samples = 0
    err_count = 0
...
def validate_requiredFields(dataF: str, setReqd):
    num_samples = 0
    num_missing_prop = 0
    with open(dataF, "r") as f:
